...
import { workflowsCatalog as workflowsData, workflowSchemas as workflowSchemasData } from "@gha-dispatcher/shared";
...
export async function registerRoutes(
  ...
  // -- Workflow schemas (audit bundle, public) ----------------------------
  // Surfaces the JSON-Schema-2020-12 bundle generated by audit_workflows.py.
  // /api/workflow-schemas         -> full bundle (stats + every workflow)
  // /api/workflows/:id/schema     -> single workflow record by id (filename stem)
  const schemasBundle = workflowSchemasData as unknown as {
    generated_at: string;
    repo: string;
  ...
  app.get("/api/workflow-schemas", (_req, res) => {
    res.json(schemasBundle);
  });

  app.get("/api/workflows/:id/schema", (req, res) => {
    const id = String(req.params.id || "").trim();
    // Strip .yml/.yaml if the client passes a filename
    const key = id.replace(/\.(ya?ml)$/i, "");
    const rec = schemasBundle.workflows[key];
    if (!rec) {
      res.status(404).json({ error: "workflow not found", id: key });
      return;
    }
    res.json(rec);
  });
